...
4. Make the smallest scoped changes that satisfy the task.
5. Run focused validation first; run broader checks only when risk or policy
   calls for them.
6. Keep the lease alive if you own one.
7. Stop after this task. Report follow-up work instead of expanding scope.
